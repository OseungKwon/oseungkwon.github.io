...
앞에서 본 두 방식을 조합하면, 대용량 파일을 메모리에 통째로 올리지 않고 다운로드할 수 있습니다. 핵심은 응답을 스트림으로 받아 도착하는 조각을 그대로 디스크에 흘려보내는 것입니다. 전체 파일을 메모리에 쌓아 두지 않으니, 파일이 커져도 사용하는 메모리는 일정하게 유지됩니다.

저장 경로는 브라우저 지원 여부에 따라 둘로 나눴습니다.

- **`showSaveFilePicker`를 지원하는 브라우저**: 사용자가 고른 위치의 파일 핸들에서 `createWritable()`로 스트림을 열고, 내려받는 조각을 `write()`로 이어 붙인 뒤 `close()`로 확정합니다. 사용자가 지정한 폴더에 곧바로 저장됩니다.
- **지원하지 않는 브라우저**: 파일 선택 UI를 쓸 수 없으므로, 권한 없이 즉시 쓸 수 있는 OPFS에 임시 파일로 먼저 저장합니다. 다운로드가 끝나면 그 파일을 읽어 Blob URL로 만들고, `<a>` 태그로 사용자에게 내려줍니다.

두 경로 모두 스트림을 받아 디스크에 쓰는 작업은 web worker에서 처리했습니다. 이 작업이 메인 스레드에서 돌면 그동안 렌더링이 밀려 UI가 멈추기 때문입니다. 쓰기를 worker로 넘기면 메인 스레드는 진행 상태만 받아 화면에 반영하면 되므로, 다운로드 중에도 화면이 부드럽게 반응합니다. 또한 다운로드가 특정 컴포넌트가 아니라 worker에서 돌기 때문에, 사용자가 다른 화면으로 이동해도 다운로드는 끊기지 않고 오버레이로 유지됩니다.
