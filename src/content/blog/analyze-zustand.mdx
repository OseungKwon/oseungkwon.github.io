...
선택자로 구독한 슬라이스가 바뀔 때만 컴포넌트가 리렌더되는 동작을 아래에서 직접 확인할 수 있습니다. 각 버튼은 서로 다른 슬라이스를 변경하며, 그 슬라이스를 구독한 컴포넌트만 리렌더됩니다.

<Embed
  src="/embeds/zustand-selector.html"
  title="선택자로 구독한 슬라이스가 바뀐 컴포넌트만 리렌더되는 모습"
/>
